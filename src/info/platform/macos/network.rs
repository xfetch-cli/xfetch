//! Network probing on macOS.
//!
//! `get_local_ip_info` in `crate::info::system` picks the first non-loopback
//! IPv4 address in HashMap order, which on macOS tends to surface virtual
//! adapters (utun from VPNs, tunnel/bridge interfaces, Tailscale) instead of
//! the physical interface (en0/en1). This module prefers a physical interface
//! and only falls back to the generic behavior when every address is virtual.
...
mod tests {
    ...
    #[test]
    fn test_is_virtual() {
        assert!(is_virtual("utun4"));
        assert!(is_virtual("tun0"));
        assert!(is_virtual("bridge0"));
        assert!(is_virtual("awdl0"));
        assert!(is_virtual("ipsec0"));
        assert!(is_virtual("ppp0"));
        assert!(is_virtual("Tailscale"));
        assert!(!is_virtual("en0"));
        assert!(!is_virtual("en1"));
    }
}
