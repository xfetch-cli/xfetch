...
fn parse_jsonc_str(content: &str) -> Option<Value> {
    // Windows editors (PowerShell 5.1 `Set-Content -Encoding UTF8`, Notepad)
    // prepend a UTF-8 BOM, which serde_json rejects; strip it so a config
    // written there is not silently replaced by the defaults.
...
mod tests {
    ...
    /// Guards the embedded config template and the default icon set against
    /// broken Unicode: a replacement character (U+FFFD), a stray control
    /// character or an emoji. Configs use Nerd Font glyphs (private use area)
    /// and plain text; emoji must never be baked into generated configs.
    #[test]
    fn test_generated_config_glyphs_are_valid() {
        ...
        fn assert_clean(label: &str, text: &str) {
            for (index, ch) in text.chars().enumerate() {
                assert_ne!(
                    ch, '\u{FFFD}',
                    "{} contains the replacement character at offset {}",
                    label, index
                );
                assert!(
                    !ch.is_control() || matches!(ch, '\n' | '\r' | '\t'),
                    "{} contains control character U+{:04X} at offset {}",
                    label,
                    ch as u32,
                    index
                );
                assert!(
                    !is_emoji(ch),
                    "{} contains emoji U+{:04X} at offset {}",
                    label,
                    ch as u32,
                    index
                );
        ...
        assert_clean("GEN_CONFIG_TEMPLATE", GEN_CONFIG_TEMPLATE);
        for (module, icon) in Config::default().icons {
            assert_clean(&format!("default icon '{}'", module), &icon);
        }
    }
    ...
    #[test]
    fn test_parse_jsonc_strips_utf8_bom() {
        // PowerShell 5.1 `Set-Content -Encoding UTF8` writes the BOM.
        let jsonc = "\u{feff}{ \"layout\": \"section\" }";
        let parsed = parse_jsonc_str(jsonc).expect("BOM-prefixed jsonc should parse");
        assert_eq!(parsed["layout"], "section");
    }
}
